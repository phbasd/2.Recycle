...
public class RecvAdapter extends RecyclerView.Adapter<RecvAdapter.PhbViewholder>{
    ...
    //ViewHolder == 몇개의 위젯이 들어있는지 OS가 판단x, 우리가 커스텀해서 사용함.
    //어떤 아이템에는 이미지뷰가 3개, 어떤 아이템에는 텍스트뷰가3개로 다를수가있음.
    //getView (getView 메소드) <==> 현) onCreateViewHolder
    @NonNull
    @Override
    public PhbViewholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        PhbViewholder viewholder = new PhbViewholder(v);
        return viewholder;
    }

    //5. 데이터가 있다면 position holder에 담아져있는 (칸) 위젯과 연결 부분 ( Binding )
    @Override
    public void onBindViewHolder(@NonNull PhbViewholder h, int i) {
    ...
    //일반==>ViewHloder타입임.
    //1.ViewHolder타입을 상속받은 innerClass ViewHolder를 만든다.(이름은 관례상ViewHolder)
    public class PhbViewholder extends RecyclerView.ViewHolder {
        ImageView imgv1, imgv2;
        Button btn_change;
        public PhbViewholder(@NonNull View v) {
            super(v);
